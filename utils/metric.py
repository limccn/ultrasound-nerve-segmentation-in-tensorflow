...
def msd(input1, input2, voxelspacing=None, connectivity=1):
    """
    Maxsim surface distance metric.
...
def __surface_distances(input1, input2, voxelspacing=None, connectivity=1):
    ...
    """
    obj1_max_distance = math.sqrt(input1.shape[0]**2+input2.shape[1]**2)/2.
    obj2_max_distance = math.sqrt(input1.shape[0]**2+input2.shape[1]**2)/2.
    ...
    # test for emptiness
    obj1 = numpy.count_nonzero(input1) 
    obj2 = numpy.count_nonzero(input2)

    if 0 == obj1 :
        #print('The first supplied array does not contain any binary object.')
        if 0 == obj2:
            #print('The second supplied array does not contain any binary object.')    
            return numpy.array([0])
        else:
            #print('return second maxium distance ')
            return numpy.array([obj2_max_distance])
    else:
        if 0 == obj2:     
            #print('The second supplied array does not contain any binary object.')
            #print('return first maxium distance ')
            return numpy.array([obj1_max_distance])
        else:
            pass
...
def main():
    ...
    mask = cv2.imread('/Users/limc/Desktop/03-step-valid/test/7_2_mask.tif',0)#
    predict_5k = cv2.imread('/Users/limc/Desktop/03-step-valid/mask-5000/7_2.pred.mask.tif',0)#
    #predict_50k = cv2.imread('/Users/limc/Desktop/03-step-valid/test/mask-50000/7_1.pred.mask.tif',cv2.IMREAD_GRAY)#
    #predict_100k = cv2.imread('/Users/limc/Desktop/03-step-valid/test/mask-100000/7_1.pred.mask.tif',cv2.IMREAD_GRAY)#
    #predict_250k = cv2.imread('/Users/limc/Desktop/03-step-valid/test/mask-250000/7_1.pred.mask.tif',cv2.IMREAD_GRAY)#


    mask = mask / 255.
    predict_5k = predict_5k / 255.

    threshold = .5
    mask[:][mask[:]<=threshold]=0
    mask[:][mask[:]>threshold]=1
    predict_5k[:][predict_5k[:]<=threshold]=0
    predict_5k[:][predict_5k[:]>threshold]=1

    mask = np.uint8(mask)
    predict_5k = np.uint8(predict_5k)

    print("DC:{:-1f}".format(dc(mask,predict_5k)))
    print("JC:{:-1f}".format(jc(mask,predict_5k)))
    print("RAVD:{:-1f}".format(ravd(mask,predict_5k)))
    print("RECALL:{:-1f}".format(recall(mask,predict_5k)))
    print("PRECISION:{:-1f}".format(precision(mask,predict_5k)))
    print("ASD:{:-1f}".format(asd(mask,predict_5k)))
    print("MSD:{:-1f}".format(msd(mask,predict_5k)))
    print("OBJASD:{:-1f}".format(obj_asd(mask,predict_5k)))
    print("ASSD:{:-1f}".format(assd(mask,predict_5k)))
    print("OBJASSD:{:-1f}".format(obj_assd(mask,predict_5k)))
    print("FPR:{:-1f}".format(obj_fpr(mask,predict_5k)))
    print("TPR:{:-1f}".format(obj_tpr(mask,predict_5k)))
    print("HD:{:-1f}".format(hd(mask,predict_5k)))
