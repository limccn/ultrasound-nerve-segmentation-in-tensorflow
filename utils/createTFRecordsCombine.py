...
# create tf writer
record_filename = '../data/tfrecords/train_combine.tfrecords'
record_count=0
...
# list of files
train_filename = glb('../data/train_combine/*') 
...
for image in image_filename:
  key = image[:-4] 
  masks=[s for s in mask_filename if key+'_mask' in s]
  if(len(masks) > 0):
    mask = masks[0]
